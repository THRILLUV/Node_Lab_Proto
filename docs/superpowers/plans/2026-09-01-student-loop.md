...
- [x] P5.2 `lib/core/mock.mjs` Fisher-Yates items + choices; answer index moves. Tests first. Commit `checkpoint: P5.2 mock mode1 shuffle`
- [x] P5.3 New `nl_sessions` row, empty attempts, original order. Commit `checkpoint: P5.3 mock mode2 clean retry`
- [x] P5.4 Variants via `api/variant` + `questions.json`. No gen key → 10-q mini. Commit `checkpoint: P5.4 mock mode3 variants`
