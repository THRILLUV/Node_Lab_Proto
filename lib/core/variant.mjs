...
export function originalForVerify(originalStem = "", itemIndex = 1) {
  const stem = String(originalStem || "").trim();
  if (stem) return stem;
  const n = Number(itemIndex);
  return `문항 ${Number.isInteger(n) && n > 0 ? n : 1}`;
}
...
export function studentVisibleVariant(variant, originalStem = "", itemIndex = 1) {
  ...
  const check = verifyVariant({
    expr_original: originalForVerify(originalStem, itemIndex),
