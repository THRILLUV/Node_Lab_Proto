...
export function onboardingPersistPayload(userId, answers = {}) {
...
export async function persistOnboarding(sb, userId, answers) {
  const payload = onboardingPersistPayload(userId, answers);
  const res = await sb.from("nl_profiles").upsert(payload);
  if (res?.error) throw res.error;
  return payload;
}
