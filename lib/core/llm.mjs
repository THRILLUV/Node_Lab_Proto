...
export function zenFreeEnabled(env = process.env) {
  if (env.OPENCODE_FREE === "0") return false;
  const testCtx = env.NODE_TEST_CONTEXT ?? process.env.NODE_TEST_CONTEXT;
  if (testCtx && env.OPENCODE_FREE !== "1") return false;
  const base = env.LLM_BASE_URL || "https://opencode.ai/zen/v1";
  return /opencode\.ai/i.test(base);
}

export function zenRequestHeaders(env = process.env) {
  const headers = {
    "content-type": "application/json",
    "user-agent": "Mozilla/5.0 (compatible; NodeLab/1.0)",
  };
...
export async function openAiChat({ prompt, imageB64, env = process.env } = {}) {
  ...
  const base = String(
    env.LLM_BASE_URL ||
      env.HF_CHAT_URL ||
      (key || zenFreeEnabled(env) ? "https://opencode.ai/zen/v1" : ""),
  ).replace(/\/$/, "");
  if (!base) return null;
  if (!key && !zenBase(base)) return null;
