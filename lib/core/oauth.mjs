...
import { supabaseAnonKey as envSupabaseAnon, supabaseUrl as envSupabaseUrl } from "./env-names.mjs";
import {
  allowedRedirect,
  authorizeUrl,
  oauthEmail,
...
export function createOAuthState({
  ...
  if (!allowedRedirect(redirect, env)) throw new Error("redirect not allowed");
  const payload = Buffer.from(JSON.stringify({ r: redirect, n: nonce, t: now })).toString("base64url");
  const sig = createHmac("sha256", secret).update(payload).digest("base64url");
  return `${payload}.${sig}`;
}

export function parseOAuthState(state, secret, now = Date.now(), env = process.env) {
  ...
  try {
    ...
    if (!allowedRedirect(data.r, env)) return null;
...
export async function handleSocialOAuth(req, res, provider, env = process.env, fetchImpl = fetch) {
  ...
  const stateSecret = env.NL_OAUTH_DB_SECRET || envSupabaseAnon(env);
  const supabaseUrl = envSupabaseUrl(env);
  const supabaseAnon = envSupabaseAnon(env);
  const appRedirect = url.searchParams.get("redirect") || `${origin}/`;

  if (!url.searchParams.get("code")) {
    if (!allowedRedirect(appRedirect, env)) return send(res, 400, { error: "redirect_not_allowed" });
    const state = createOAuthState({ redirect: appRedirect, secret: stateSecret, env });
  ...
  try {
    const parsed = parseOAuthState(url.searchParams.get("state"), stateSecret, Date.now(), env);
    ...
    const dest = allowedRedirect(appRedirect, env) ? appRedirect : `${origin}/`;
