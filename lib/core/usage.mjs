...
export function usageBarView({ used = 0, limit = 1 } = {}) {
  const n = Math.max(0, Number(used) || 0);
  const cap = Number(limit);
  if (!Number.isFinite(cap) || cap <= 0) {
    return { percent: 100, copy: `손풀이 ${n}/한도`, credit: false };
  }
  const remaining = Math.max(0, cap - n);
  const percent = Math.max(0, Math.min(100, (remaining / cap) * 100));
  return { percent, copy: `손풀이 ${n}/${cap}`, credit: false };
}

export function studentPlanName(plan = "") {
  const key = String(plan || "").trim();
  const lower = key.toLowerCase();
  if (lower === "pro" || key === "베이직" || lower === "basic") return "베이직";
  if (lower === "heavy" || key === "헤비") return "헤비";
  if (lower === "free" || lower === "light" || key === "라이트") return "라이트";
  if (lower === "guest" || key === "비회원") return "비회원";
  return "비회원";
}
